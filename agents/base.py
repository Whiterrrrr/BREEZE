...
class AbstractBRO(torch.nn.Module, metaclass=abc.ABCMeta):
    """Abstract base class for all feedforward networks."""
    ...
    def __init__(
        ...
        super().__init__()
        self.function_list = self._build()

    def _build(self) -> torch.nn.Sequential:
        """
        Creates MLP trunk.
        """
        assert self.hidden_layers != 0, "hidden_layers != 0"
        # first layer
        # ICLR paper uses layer norm and tanh for first layer of every network
        function_1st = torch.nn.Sequential(
            torch.nn.Linear(self.input_dimension, self.hidden_dimension),
            torch.nn.LayerNorm(self.hidden_dimension),
            torch.nn.ReLU()
        )
        function_last = torch.nn.Linear(self.hidden_dimension, self.output_dimension)
        function_res = torch.nn.ModuleList()
        ...
        function_list = {
            "function_1st": function_1st.to(self.device),
            "function_res": function_res.to(self.device),
            "function_last": function_last.to(self.device),
        }
...
class OfflineReplayBuffer(AbstractOfflineReplayBuffer):
    ...
    def _init_stat(self):
        self.dataset_num = self.storage["observations"].shape[0]
        self.s_mean, self.a_mean = torch.tensor([0]).to(self.device), torch.tensor([0]).to(self.device)
        self.s_std, self.a_std = torch.tensor([1]).to(self.device), torch.tensor([1]).to(self.device)

    def _stat(self):
        self.s_mean = self.storage["observations"].mean(axis=0, keepdims=True)
        self.a_mean = self.storage["actions"].mean(axis=0, keepdims=True)
        self.s_std = self.storage["observations"].std(axis=0, keepdims=True)
        self.a_std = self.storage["actions"].std(axis=0, keepdims=True)

    def _normalize_dataset(self):
        self.storage["observations"] = (self.storage["observations"] - self.s_mean) / (self.s_std + EPS)
        self.storage["next_observations"] = (self.storage["next_observations"] - self.s_mean) / (self.s_std + EPS)
        self.storage["actions"] = (self.storage["actions"] - self.a_mean) / (self.a_std + EPS)
        self.storage["next_actions"] = (self.storage["next_actions"] - self.a_mean) / (self.a_std + EPS)
    ...
    def load_offline_dataset(
        ...
        for _, episode in tqdm(episodes.items(), desc="Loading episodes from buffer"):
            ...
            # store in lists
            observations.append(
                torch.as_tensor(episode["observation"][:-2], device=self.device)
            )
            actions.append(torch.as_tensor(episode["action"][1:-1], device=self.device))
            rewards.append(torch.as_tensor(episode["reward"][1:-1], device=self.device))
            next_observations.append(
                torch.as_tensor(episode["observation"][1:-1], device=self.device)
            )
            next_actions.append(torch.as_tensor(episode["action"][2:], device=self.device))
            discounts.append(
                torch.as_tensor(
                    episode["discount"][2:] * self._discount, device=self.device
                )
            )
            physics.append(np.array(episode["physics"][:-2]))
        ...
        if action_condition is not None:
            ...
            self.storage["next_actions"] = self.storage["next_actions"][action_condition_idxs]

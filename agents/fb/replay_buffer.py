...
class FBReplayBuffer(OfflineReplayBuffer):
    ...
    def sample_task_inference_transitions(
        ...
        a_mean, a_std = self.a_mean.detach().cpu().numpy(), self.a_std.detach().cpu().numpy()
        
        batch_physics = self.storage["physics"][batch_indices]
        batch_actions = self.storage["actions"][batch_indices].detach().cpu().numpy() * (a_std  + EPS) + a_mean
        ...
        z_inf_observations = (z_inf_observations - self.s_mean) / (self.s_std + EPS)
